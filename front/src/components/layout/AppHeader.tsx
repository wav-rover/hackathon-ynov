...
export function AppHeader() {
  ...
  const accountHref = buildPath(isAuthenticated ? "/account" : "/signup")

  return (
    <header className="shrink-0 border-b border-border bg-background">
      <div className="flex h-14 items-center gap-3 px-4 sm:h-16 sm:gap-6 sm:px-6">
        <Link to={buildPath("/")} className="min-w-0 shrink">
        ...
        <SheetContent side="right" className="w-full gap-0 p-0 sm:max-w-xs">
          ...
          <div className="flex flex-col gap-2 px-4 py-4">
            ...
            <nav
              ...
              className="mt-2 flex flex-col gap-1 border-t border-border pt-4"
            >
              <NavLink
                to={buildPath("/")}
                className={({ isActive }) =>
                  cn(getNavLinkClass(isActive), "px-2 py-2.5")
                }
                onClick={() => setIsMenuOpen(false)}
              >
                Find a Vet
